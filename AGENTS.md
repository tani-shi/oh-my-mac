...
- Config sync has a prepare phase and an apply phase. The prepare phase validates every local input, generated artifact, supported syntax, ownership conflict, and required command without changing managed destinations or deleting orphans. The apply phase uses only prepared outputs, atomically replaces each file through a temporary path on its destination filesystem, and preserves an existing destination's mode.
- `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` are generated: preparation concatenates `config/agents/instructions.md` with the CLI's own `instructions.md` into temporary files before either destination is changed.
...
- The apply phase deletes files under `~/.claude/agents/`, `~/.claude/scripts/`, and `~/.claude/skills/` that the repository no longer declares, then removes directories left behind by a renamed or deleted skill. Deletion starts only after the complete local validation succeeds and is permanent: keep anything worth surviving a sync in `config/claude/`.
- Codex skill ownership is recorded as relative file paths in `~/.agents/skills/.oh-my-mac-managed` because that directory is shared with independently installed skills. Sync aborts rather than overwrite an unrecorded same-name skill, removes only valid paths in the previous manifest that no longer exist under `config/codex/skills/`, and updates the manifest after those deletions so a failed run can retry them. Unrecorded skills and files are left untouched.
- External state changes are applied after managed files and orphan reconciliation. A failure identifies its target and everything already applied, exits nonzero, and leaves the run convergent; the Sheldon lock retry marker remains until `sheldon lock --update` succeeds.
- Global git config, including aliases such as `git discard`, is managed via the `git_config_keys` array as `"key:value"`. A script an alias invokes is synced through the `configs` array and run as `zsh <path>`, so it needs no executable bit; file sync preserves existing modes and manages content only.
