...
- Every Git requirement in `config/uv/tools.txt` uses a full commit SHA. Update `agent-sentinel` only through `make refresh-agent-sentinel`; the routine upgrade workflow may update the other reviewed uv tool pins.
