...
- **uv tools**: Tools in `config/uv/tools.txt` MUST use `@tag` or `@commit` suffix, except `agent-sentinel` and `claude-sessions` (owned by the user, always use HEAD).
...
- **Claude Code plugins**: `config/claude/plugins.txt` declares the user-scope plugin set but does not pin plugin versions. `make install`/`make update` install missing declarations and uninstall undeclared user-scope plugins without updating installed ones. After the user approves an `/upgrade` plan, `make upgrade-apply` explicitly updates every declared plugin and stops before later tools if any update fails.
