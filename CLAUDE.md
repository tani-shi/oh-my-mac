...
- **mise tools**: Tools in `config/mise/config.toml` MUST use an exact version (e.g., `dotnet = "10.0.301"`), never a floating channel. The .NET SDK is managed here; `mise install` runs during `make install`/`make update`. Per-project `global.json` selects the SDK among side-by-side versions in `DOTNET_ROOT`.
