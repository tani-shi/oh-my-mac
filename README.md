...
`config/codex/skills/deliver-change/` supervises one requested change from implementation through deployment verification. Run `$deliver-change <objective>` with either a GitHub issue reference or a plain-language request to start. The invoking Codex GUI task creates a separate worktree task, waits for its pull request, and privately routes review revisions until the change is ready for human verification; GitHub review comments and replies remain human-authored. The skill always stops before merge with the implementation task still available. After verifying the pull request, run `$deliver-change` again in the same supervisor task to reconcile the current state, merge when every gate is satisfied, archive the implementation task, clean up the branch, and perform applicable post-deployment verification. Repeated invocations execute only incomplete transitions. Implicit invocation is disabled.
