...
| Claude Code | `config/claude/version` contains an exact version. | Uses `claude install` for that version and verifies the installed version. A failed installer stops convergence without running a remote fallback script. | The reviewed version replaces the pin and is then installed and verified. | The direct installed version is fixed. Installer artifacts are not hashed or vendored here. |
...
| uv tools | Every Git requirement in `config/uv/tools.txt`, including `agent-sentinel` and `claude-sessions`, records a full commit SHA. | Installs the declared requirement and reports each Git commit in the installation log. A declaration without a full SHA stops convergence. | The routine workflow may review and replace ordinary uv tool pins. `agent-sentinel` advances only through `make refresh-agent-sentinel`, which fetches and verifies the upstream commit before writing its pin. | The named Git source commits are fixed. Python versions, transitive dependencies, build inputs, and artifacts are not locked here. |
...
| `make refresh-agent-sentinel` | Verify upstream agent-sentinel, advance its commit pin, and refresh generated config |
