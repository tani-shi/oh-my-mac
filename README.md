...
`make sync-config` first prepares every copied, generated, and merged file in temporary storage and validates local inputs, configuration syntax, ownership conflicts, and required commands. A local validation failure leaves managed files and orphans unchanged. The apply phase replaces files atomically through temporary paths on each destination filesystem, then removes validated orphans. External state changes cannot always be rolled back together; if one fails, the sync identifies both the failed target and targets already applied, then exits nonzero. Rerunning the sync applies only the remaining work, including a pending Sheldon lock update recorded after a previous failure.

| Category | `make sync-config` behavior |
| --- | --- |
| File copies | Atomically replaces regular files from `config/`, concatenated shared and CLI-specific instruction files, and Codex configuration generated by agent-sentinel only when their contents differ. Existing destination modes are preserved. |
