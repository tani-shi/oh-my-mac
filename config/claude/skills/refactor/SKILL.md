...
- **PLAUSIBLE** — the flaw is real but the fix needs context beyond the reviewed files (callers, tests, API stability). State what would confirm it. A mechanical, behavior-preserving follow-through on the callers of an in-scope change — e.g. rewriting imports after a rename, move, or consolidation — is not such context: read those callers to construct the refactoring, and do not cap at PLAUSIBLE merely because they lie outside the diff.
