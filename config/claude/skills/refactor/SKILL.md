...
**Skip** (full-scope, checked *before* base resolution because it is base-independent — a match spares the `gh pr view` call). Only when the marker `.git/refactor-mark` (`git rev-parse --git-path refactor-mark`) exists: compute the working-tree tree SHA `cur`, and if it equals the marker's contents, report that nothing changed since the last full review and stop — do not run Phases 1–5. Compute `cur` through a throwaway index so the real index is untouched (this computation is a contract — do not alter it):
...
- **No base** — none of the above resolved: review the working tree only, and note this in the Phase 5 report.
...
- **Single-source test** — a comment that exists only to keep duplicated or double-managed state in sync is masking a structural flaw, not carrying why-not. Tag it for Angle C's consolidation, or, if the restructuring lies outside the reviewed files, report it as an out-of-scope opportunity — never keep it, and never route it to PLAUSIBLE, which is reserved for findings that turn on human judgment.
...
Tests catch behavior regressions but not a green-but-wrong reshaping, so a `restructure` reaches this apply step only by passing the Phase 2 proof gate; one that cannot be proven behavior-preserving never applies here — it caps at PLAUSIBLE and goes to the human in Phase 4 rather than being committed blind.

## Phase 4 — Adjudicate

This phase fires only when the session can put a question to a human. Under headless, piped, or `-p` invocation it is skipped: every PLAUSIBLE finding stays in the Phase 5 report unchanged, which keeps the skill portable. Gate on whether a human can be reached, not on any brittle environment probe.

The orchestrator (main loop) runs this phase — it, not the Explore verifiers of Phases 1–2, is what can call AskUserQuestion. Present the **actionable** PLAUSIBLE findings as the options of multi-select questions within a single AskUserQuestion call — findings are options, never a question apiece, which is the interrogation this batching exists to avoid. The call holds up to four questions of up to four options each; rank findings by value and group them into questions by file or theme. Findings past that sixteen-option ceiling are **deferred** to the Phase 5 report, not dropped silently. For each finding the human selects, loop back into the Phase 3 apply procedure (behavior-preserving, running the project's tests or type checks). A finding left unselected is **declined** — the human saw it and ruled, so it goes to the report but, unlike a deferred one, does not block the mark.

## Phase 5 — Report

Summarize in the final message: applied refactorings (`file:line — refactoring — comment removed`), separating those applied directly (Phase 3) from those applied after the human approved them in Phase 4; the PLAUSIBLE findings left for the user — every FYI finding, plus the actionable ones the human declined or that were deferred (all actionable findings when Phase 4 was skipped) — each with the judgment at stake; and REFUTED comments that earned their place (one line each). If nothing was found, say the reviewed scope is clean and what was checked.

**Record the mark** — only in full-scope with a resolved base (not the working-tree-only fallback), only once Phases 1–5 have completed, and only when no actionable PLAUSIBLE finding was left un-adjudicated: recompute the working-tree tree SHA `cur` with the Phase 0 skip block and write it to `.git/refactor-mark` (`git rev-parse --git-path refactor-mark`). Recompute here rather than reusing the Phase 0 value because Phases 3 and 4 may have changed the tree. This base-required record is deliberately asymmetric with the base-independent skip: a degraded working-tree-only scope leaves no mark, so it can never claim a full review it did not perform. The same asymmetry covers un-adjudicated work: a run that skipped Phase 4 (headless) or deferred actionable findings past the question cap leaves findings the human never ruled on — record no mark, so the next full-scope run re-surfaces them instead of skipping on an unchanged tree. Findings the human saw and declined are ruled on, and do not block the mark.
